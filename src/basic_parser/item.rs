...
impl Parser<Item> for Item {
    fn parse<P: Into<std::path::PathBuf>>(path: P) -> anyhow::Result<Item> {
        ...
        let data_content = std::fs::read_to_string(&data_path).with_context(|| {
            format!("Failed to read contents for file: {}", data_path.display())
        })?;
...
for path in asset_paths {
    let item = Item::parse(path)?; // Items/Food/Cheese
                                               // ^ stem
